...
- `create`: ZIP, TAR, TAR.GZ, TAR.BZ2, TAR.XZ 생성
- `extract`: ZIP, ZIP 분할 볼륨, TAR 계열, GZ/BZ2 stream, libarchive가 읽을 수 있는 일부 컨테이너 해제
...
./bin/newnzip-engine create --format=zip --split=100m output.zip /path/to/folder
...
기본 macOS 환경에서는 `bsdtar`, `gzip`, `bzip2`를 사용합니다. 다음 형식은 실행파일이 설치되어 있거나 앱에 번들되어 있어야 동작합니다.
